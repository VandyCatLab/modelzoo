...
if os.path.exists('../outputs/tracker'+i+'.npy'):
    tracker = np.load('../outputs/tracker'+i+'.npy')
else:
    tracker = np.zeros((100, 100), dtype='uint8')
...
for w in range(int(i), 10):
    ...
    for s in range(10):
        ...
        np.save('../outputs/tracker'+i+'.npy', tracker)
        model.save('../outputs/models/ten_by_ten/w'+str(w)+'s'+str(s)+'.npy')
