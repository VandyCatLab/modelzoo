...
class Trajectory_Callback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch in [0, 1, 2, 3, 4, 5,
            ...
            print('\n\nSnapshot seeds', str(weight_seed), str(shuffle_seed), 'at epoch', str(int(epoch)+1))
...
num_trained = 0
weight_seed = int(sys.argv[1])
shuffle_seed = int(sys.argv[2])
total = int(sys.argv[3])
abort = False
while num_trained < total:
    print('Training All_CNN_C with weight seed', weight_seed, 'and shuffle seed', shuffle_seed)
    K.clear_session()
    ...
    model = init_model('all_cnn_c', seed=weight_seed)
